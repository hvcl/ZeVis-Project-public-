...
public:
	...
	void SetData(const float *inputData, int NumberVectors, int InputVectorSize) {
		MatrixOfVectors InputVectors(NumberVectors, 1);
		...
		for (int i = 0; i < NumberVectors; i++) {
			...
			for (int j = 0; j < InputVectorSize; j++) {
				InputVector(j) = inputData[i*InputVectorSize + j];
				//std::cout << cell <<std::endl;
				if (inputData[i*InputVectorSize + j] > GetInputVectors::MaxValueInputData) { GetInputVectors::MaxValueInputData = inputData[i*InputVectorSize + j]; }
			}
			InputVectors(i, 0) = InputVector;
		}
		GetInputVectors::InputVectors = InputVectors;
